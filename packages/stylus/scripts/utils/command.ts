...
import type { DeployOptions, DeploymentConfig } from "./type";
...
export async function buildDeployCommand(config: DeploymentConfig, deployOptions: DeployOptions) {
...
export async function estimateGasPrice(config: DeploymentConfig, deployOptions: DeployOptions): Promise<string> {
    ...
    childProcess.on("close", (code: number | null) => {
      ...
      if (code === 0 && !errors) {
        ...
        if (errors) {
          ...
          if (!command.includes("--no-verify") && errors.includes("mismatch number of constructor arguments")) {
...
function extractErrorLines(errorLines: string[]): string | null {
  ...
  if (errorLines.length > 0) {
    ...
    const errorIndex = errorLines.findIndex(
      line => line.toLowerCase().includes("error[") || line.toLowerCase().includes("error:"),
    );
