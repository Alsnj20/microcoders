...
import type { DeploymentData, ExportConfig } from "./type";
...
export function getExportConfig(contractFolder?: string, contractName?: string, chainId?: string): ExportConfig {
  if (!contractFolder) {
  ...
  const deploymentData = getContractDataFromDeployments(deploymentDir, contractName, chainId);
  if (!deploymentData) {
    throw new Error(
...
export function isContractFolder(contractFolder: string): boolean {
  ...
  return fs.existsSync(cargoTomlPath) && fs.existsSync(libRsPath) && fs.existsSync(mainRsPath);
}
...
export function handleSolcError(error: Error, context = "ABI export"): void {
  console.error(`\n❌ ${context} failed!`);
...
export function getContractData(chainId: string, contractName: string) {
  ...
  if (!deployedContracts || !deployedContracts[chainId] || !deployedContracts[chainId][contractName]) {
