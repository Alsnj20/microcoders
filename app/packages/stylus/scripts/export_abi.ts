...
import { ensureDeploymentDirectory, executeCommand, generateTsAbi, getExportConfig, handleSolcError } from "./utils/";

export async function exportStylusAbi(contractFolder: string, contractName: string, isScript = true, chainId?: string) {
  ...
  try {
    ...
    // do not Generate TypeScript ABI when called from pnpm script
    if (!isScript) {
