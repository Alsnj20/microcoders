...
export function ChatContextPanel({
  ...
  return (
    <aside className="hidden xl:flex w-72 flex-col h-full border-l border-border bg-background overflow-hidden">
      {/* Contexto Activo */}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {linkedMemories.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">
            No hay memorias vinculadas a este agente.
          </p>
        ) : (
          <div className="space-y-2">
            {linkedMemories.map((mem) => (
              <div
                key={mem.memoryId}
                className="p-2.5 rounded-lg bg-muted border border-border"
              >
